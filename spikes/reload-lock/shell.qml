...
// Throwaway spike for issue #57 — see README.md. This is its own config root, so
// it deliberately does not `import qs.*` or use the `Appearance` singleton: pulling
// the real config in would put the live shell's widgets on screen and make the
// reload under test a reload of the whole config rather than of the lock alone.
ShellRoot {
    id: root
